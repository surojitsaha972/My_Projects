...
# Form implementation generated from reading ui file 'reportFineMonth.ui'
#
...
class Ui_reportFineMon(object):
    def setupUi(self, reportFineMon):
        reportFineMon.setObjectName("reportFineMon")
        reportFineMon.resize(800, 600)
        self.centralwidget = QtWidgets.QWidget(reportFineMon)
        ...
        self.frame_2.setMinimumSize(QtCore.QSize(641, 0))
        ...
        self.pdf.setMinimumSize(QtCore.QSize(95, 0))
        self.pdf.setMaximumSize(QtCore.QSize(16777215, 37))
        ...
        reportFineMon.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(reportFineMon)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 800, 21))
        self.menubar.setObjectName("menubar")
        reportFineMon.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(reportFineMon)
        self.statusbar.setObjectName("statusbar")
        reportFineMon.setStatusBar(self.statusbar)

        self.retranslateUi(reportFineMon)
        QtCore.QMetaObject.connectSlotsByName(reportFineMon)

        self.fine()
        self.pdf.clicked.connect(reportFineMon.close)
        self.pdf.clicked.connect(self.repPDF)

    def retranslateUi(self, reportFineMon):
        _translate = QtCore.QCoreApplication.translate
        reportFineMon.setWindowTitle(_translate("reportFineMon", "Monthly Fine"))
        self.label.setText(_translate("reportFineMon", "Monthly Fine Report"))
        item = self.table.horizontalHeaderItem(0)
        item.setText(_translate("reportFineMon", "Std Roll"))
        item = self.table.horizontalHeaderItem(1)
        item.setText(_translate("reportFineMon", "AccessionNo"))
        item = self.table.horizontalHeaderItem(2)
        item.setText(_translate("reportFineMon", "IssueDate"))
        item = self.table.horizontalHeaderItem(3)
        item.setText(_translate("reportFineMon", "ReturnDate"))
        item = self.table.horizontalHeaderItem(4)
        item.setText(_translate("reportFineMon", "FineID"))
        item = self.table.horizontalHeaderItem(5)
        item.setText(_translate("reportFineMon", "Fine Amount"))
        self.pdf.setText(_translate("reportFineMon", "Generate PDF"))
    ...
    def fine(self):
        ...
        cur = con.cursor()
        q = "select ib.SID,ib.BookID,IssueDate,ReturnDate,ib.FineID,FineAmount from issueBook ib, fine fi where ib.FineID != ? and ib.FineID=fi.FineID and fi.Date like ?"
        cur.execute(q,("NULL",dtm))
        res= cur.fetchall()
        if res == []:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Critical)
            msg.setText("No fine has taken in this month till now")
...
if __name__ == "__main__":
    ...
    reportFineMon = QtWidgets.QMainWindow()
    ui = Ui_reportFineMon()
    ui.setupUi(reportFineMon)
    reportFineMon.show()
    sys.exit(app.exec_())
