...
# Form implementation generated from reading ui file 'reportBookStock.ui'
#
...
class Ui_bookStock(object):
    def setupUi(self, bookStock):
        bookStock.setObjectName("bookStock")
        bookStock.resize(800, 600)
        self.centralwidget = QtWidgets.QWidget(bookStock)
        ...
        self.pdf.setMinimumSize(QtCore.QSize(0, 0))
        self.pdf.setMaximumSize(QtCore.QSize(118, 35))
        ...
        bookStock.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(bookStock)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 800, 21))
        self.menubar.setObjectName("menubar")
        bookStock.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(bookStock)
        self.statusbar.setObjectName("statusbar")
        bookStock.setStatusBar(self.statusbar)

        self.retranslateUi(bookStock)
        QtCore.QMetaObject.connectSlotsByName(bookStock)

        self.stock()
        self.pdf.clicked.connect(self.repPDF)
        self.pdf.clicked.connect(bookStock.close)

    def retranslateUi(self, bookStock):
        _translate = QtCore.QCoreApplication.translate
        bookStock.setWindowTitle(_translate("bookStock", "BookStock"))
        self.label.setText(_translate("bookStock", "Book Stock Report"))
        item = self.table.horizontalHeaderItem(0)
        item.setText(_translate("bookStock", "SubjectName"))
        item = self.table.horizontalHeaderItem(1)
        item.setText(_translate("bookStock", "BookName"))
        item = self.table.horizontalHeaderItem(2)
        item.setText(_translate("bookStock", "AuthorName"))
        item = self.table.horizontalHeaderItem(3)
        item.setText(_translate("bookStock", "Publisher"))
        item = self.table.horizontalHeaderItem(4)
        item.setText(_translate("bookStock", "Edition"))
        item = self.table.horizontalHeaderItem(5)
        item.setText(_translate("bookStock", "Stock"))
        self.pdf.setText(_translate("bookStock", "Generate PDF"))

    def stock(self):
        con = sqlite3.connect("projectDB.db")
        cur = con.cursor()
        q = "select * from bookStock"
        cur.execute(q)
        res= cur.fetchall()
        if res == []:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Critical)
            msg.setText("Book stock is empty")
    ...
    def repPDF(self):
        ...
        data.insert(0,('SubjectName','BookName','AuthorName','Publisher','Edition','Stock'))
        pdf = FPDF('L','mm','A4')
...
if __name__ == "__main__":
    ...
    bookStock = QtWidgets.QMainWindow()
    ui = Ui_bookStock()
    ui.setupUi(bookStock)
    bookStock.show()
    sys.exit(app.exec_())
